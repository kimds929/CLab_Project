...
simul = TemporalMapSimulation(temporal_graph)
target_time_safe_margin = 13      # 심리적 허용시간 safe margin
time_interval = 3


history = []
df_normal = pd.DataFrame()
df_large_residual = pd.DataFrame()
df_large_api_call = pd.DataFrame()
df_after_leaving = pd.DataFrame()
...
for episode in tqdm(range(num_episodes)):
    ...
    n_api_call = 1
    context = simul.run()
    contexts_df = pd.DataFrame(simul.history_time_machine)
    contexts_df['round'] = 1

    cur_time = simul.cur_time
    path_time = simul.path_time_TimeMachine
    # 최초 API Call시간 산정
    api_call_time = int(np.ceil(simul.target_time - max(path_time * 1.5, path_time + target_time_safe_margin)))
    contexts_df['next_api_call'] = format_time_to_str(api_call_time)
    
    while(True):
        leaving_time = simul.target_time - target_time_safe_margin - path_time
        # print(f"{cur_time} / {leaving_time:.2f} / {path_time:.2f}")        
        if leaving_time < cur_time + 3:
            
            if leaving_time < cur_time:
                cur_time = cur_time
            elif leaving_time < cur_time+1:
                cur_time = cur_time + 1
            elif leaving_time < cur_time+2:
                cur_time = cur_time + 2
            elif leaving_time < cur_time+3:
                cur_time = cur_time + 3
            
            adj_mat_rule_true = simul.temporal_graph.transform(cur_time)
            api_dict_rule_true = simul.api_call(adj_mat_rule_true, save_instance=False, save_history=False)
            path_time = api_dict_rule_true['path_time']

            arrival_residual = cur_time + path_time - simul.target_time
            history_t['n_api_call'] = n_api_call
            history_t['arrival_residual'] = arrival_residual
            history.append(history_t)
            
            # save context ------------------------------------------------------------
            context = simul.save_data('history', save_data=False, save_plot=False)
            format_time_to_str(cur_time)
            context['group'] = 'history'
            context['cur_time'] = format_time_to_str(cur_time)
            context['round'] = contexts_df['round'].max()+1
            context['call_time_LastAPI'] = format_time_to_str(cur_time)
            context['call_node_LastAPI'] = simul.cur_node
            context['call_point_LastAPI'] = simul.cur_point
            context['path_LastAPI'] = api_dict_rule_true['path']
            context['path_time_LastAPI'] = api_dict_rule_true['path_time']
            context['req_leaving_time'] = format_time_to_str(int(np.ceil(leaving_time)))
            context['event'] = 'req_leaving_time'
            context['next_api_call'] = None
            contexts_df = pd.concat([contexts_df, pd.Series(context).to_frame().T], axis=0)
            # ---------------------------------------------------------------------------

            break
        else:
            cur_time = max(cur_time+3, api_call_time)

        adj_mat_rule = simul.temporal_graph.transform(cur_time)
        api_dict_rule = simul.api_call(adj_mat_rule, save_instance=False, save_history=False)
        path_time = api_dict_rule['path_time']
        api_call_time = int(np.ceil(simul.target_time - max(path_time * 1.5, path_time + target_time_safe_margin)))
        n_api_call += 1
        ...
        if leaving_time > cur_time:
            ...
            context['group'] = 'history'
            context['cur_time'] = format_time_to_str(cur_time)
            context['round'] = n_api_call
            context['call_time_LastAPI'] = format_time_to_str(cur_time)
            context['call_node_LastAPI'] = simul.cur_node
            context['call_point_LastAPI'] = simul.cur_point
            context['path_LastAPI'] = api_dict_rule['path']
            context['path_time_LastAPI'] = api_dict_rule['path_time']
            context['req_leaving_time'] = format_time_to_str(int(np.ceil(leaving_time)))
            context['event'] = ""
            context['next_api_call'] = format_time_to_str(max(cur_time+3, api_call_time))
            contexts_df = pd.concat([contexts_df, pd.Series(context).to_frame().T], axis=0)
        # ---------------------------------------------------------------------------

    # save history ------------------------------------------------------------
    if arrival_residual > 0:
        df_after_leaving = pd.concat([df_after_leaving, contexts_df], axis=0)
    elif arrival_residual < -15:
        df_large_residual = pd.concat([df_large_residual, contexts_df], axis=0)
    elif n_api_call >= 10:
        df_large_api_call = pd.concat([df_large_api_call, contexts_df], axis=0)
    elif (arrival_residual > -13) and (n_api_call <= 2):
        df_normal = pd.concat([df_normal, contexts_df], axis=0)
    # -----------------------------------------------------------------------

# 도착시간 이후
# df_after_leaving.to_clipboard()
d0 = df_after_leaving[df_after_leaving['event'] == 'req_leaving_time']
d0_excess = d0['cur_time'].apply(format_str_to_time) + d0['path_time_LastAPI'] - d0['target_time'].apply(format_str_to_time)
df_after_leaving[df_after_leaving['id'].isin(d0[d0_excess > 5].sample(10)['id'].to_list())].to_clipboard()


# 13분 이전 도착
# df_large_residual.to_clipboard()
d1 = df_large_residual[df_large_residual['event'] == 'req_leaving_time']
d1_excess = d1['cur_time'].apply(format_str_to_time) + d1['path_time_LastAPI'] - d1['target_time'].apply(format_str_to_time)
df_large_residual[df_large_residual['id'].isin(d1.sample(10)['id'].to_list())].to_clipboard()

# API Call 10회 이상
d2 = df_large_api_call[df_large_api_call['event'] == 'req_leaving_time']
d2_excess = d2[d2['round'] > 10].sample(10)
df_large_api_call[df_large_api_call['id'].isin(d2_excess['id'].to_list())].to_clipboard()

# 정상
d3 = df_normal[df_normal['event'] == 'req_leaving_time']
df_normal[df_normal['id'].isin(d3.sample(10)['id'].to_list())].to_clipboard()
...
# cPickle.dump(history, open(f"{path}/240923_RuleBased_Result_10000.pkl", 'wb'))
# cPickle.dump(history, open(f"{path}/240923_RuleBased_Result_6.5_10000.pkl", 'wb'))

history = cPickle.load(open(f"{path}/240923_RuleBased_Result_10000.pkl", 'rb'))
# history = cPickle.load(open(f"{path}/240914_RuleBased_Result_8_10000.pkl", 'rb'))

# result analysis
history_df = pd.DataFrame(history)
history_df['n_api_call'].mean()
result = (history_df['arrival_residual'] > -13) & (history_df['arrival_residual'] < 0)

# result
(history_df['arrival_residual'] > 0).sum() / len(result)    # 0.0035
result.sum()/ len(result)       # 0.8925

# result visualize
plt.hist(history_df['arrival_residual'], bins=30, color='skyblue', edgecolor='gray')
plt.axvline(0, color='red', alpha=0.5)
plt.axvline(-13, color='red', alpha=0.5)
plt.show()

plt.hist(history_df['n_api_call']-1, bins=30, color='skyblue', edgecolor='gray')
plt.show()
...
###########################################################################################################
###########################################################################################################
# TimeMachine Leaving with No API Call

from tqdm.auto import tqdm

target_time_safe_margin = 6.5
simul = TemporalMapSimulation(temporal_graph)
...
for episode in tqdm(range(num_episodes)):
    ...
    api_call_time_TM = int(np.ceil(simul.target_time - target_time_safe_margin - path_time))

    adj_mat_TM = simul.temporal_graph.transform(int(np.ceil(api_call_time_TM)))
    api_dict_TM = simul.api_call(adj_mat_TM, save_instance=False, save_history=False)


    arrival_residual = api_call_time_TM + api_dict_TM['path_time'] - simul.target_time
...
path = r'D:\DataScience\SNU_DataScience\SNU_OhLAB\CLab_Project\Model'
# path = "/content/drive/MyDrive/SNU_OhLAB/CLabProject"
# cPickle.dump(history, open(f"{path}/240914_TimeMachine_Result_10000.pkl", 'wb'))
# cPickle.dump(history, open(f"{path}/240914_TimeMachine_Result_8_10000.pkl", 'wb'))
# cPickle.dump(history, open(f"{path}/240914_TimeMachine_Result_6.5_10000.pkl", 'wb'))

# history = cPickle.load(open(f"{path}/240914_TimeMachine_Result_10000.pkl", 'rb'))
# history = cPickle.load(open(f"{path}/240914_TimeMachine_Result_8_10000.pkl", 'rb'))
...
# result
(history_df['arrival_residual'] > 0).sum() / len(result)    # 0.6069
result.sum()/ len(result)       # 0.3846
...
###########################################################################################################
# Feature-Set Version 1-5
import torch
import torch.nn as nn
import torch.optim as optim

# import requests
# response_files = requests.get("https://raw.githubusercontent.com/kimds929/CodeNote/main/42_Temporal_Spatial/DL13_TemporalSpatial_CombineModel.py")
...
with httpimport.remote_repo(f"{remote_url}/CodeNote/main/42_Temporal_Spatial/"):
    from DL13_TemporalSpatial_CombineModel import CombinedEmbedding, EnsembleCombinedModel, make_feature_set_embedding
...
# temporal_feature_cols = ['cur_time','target_time', 'call_time_TimeMachine']
temporal_feature_cols = ['cur_time','target_time', 'call_time_LastAPI', 'call_time_TimeMachine', 'remain_time_from_TimeMachine']
spatial_feature_cols = ['cur_point', 'target_point']
other_feature_cols = ['path_time_TimeMachine', 'path_time_LastAPI']
...
apicall_0_model = EnsembleCombinedModel(input_dim=input_dim, output_dim=1, t_input_dim=len(temporal_feature_cols),
                hidden_dim=hidden_dim,  n_layers=3, n_models = 10, n_output=2,
                embed_output_dim=64, t_hidden_dim=32, s_emb_dim=32, t_emb_dim=17,
                coord_embed_dim=16, coord_hidden_dim=32, coord_depth=2, grid_size=32, periodic_embed_dim=17)

apicall_1_model = EnsembleCombinedModel(input_dim=input_dim, output_dim=1, t_input_dim=len(temporal_feature_cols),
                hidden_dim=hidden_dim,  n_layers=3, n_models = 10, n_output=2,
                embed_output_dim=64, t_hidden_dim=32, s_emb_dim=32, t_emb_dim=17,
                coord_embed_dim=16, coord_hidden_dim=32, coord_depth=2, grid_size=32, periodic_embed_dim=17)
# apicall_0_model(torch.rand(5,input_dim))
# apicall_1_model(torch.rand(5,input_dim))
# sum(p.numel() for p in apicall_0_model.parameters() if p.requires_grad)     # 62724
# sum(p.numel() for p in apicall_1_model.parameters() if p.requires_grad)     # 62724

model_pathtime = EnsembleCombinedModel(input_dim=input_dim, output_dim=1, t_input_dim=len(temporal_feature_cols),
                hidden_dim=hidden_dim,  n_layers=3, n_models = 10, n_output=1,
                embed_output_dim=64, t_hidden_dim=32, s_emb_dim=32, t_emb_dim=17,
                coord_embed_dim=16, coord_hidden_dim=32, coord_depth=2, grid_size=32, periodic_embed_dim=17)
# model_pathtime(torch.rand(5,input_dim))
# sum(p.numel() for p in model_pathtime.parameters() if p.requires_grad)     # 61434


apicall_0_model.to(device)
apicall_1_model.to(device)
model_pathtime.to(device)

# compile -------------------------------------------------------------------------------------
alpha = 1

optimizer_apicall_0 = optim.Adam(apicall_0_model.parameters(), lr=1e-3)
optimizer_apicall_1 = optim.Adam(apicall_1_model.parameters(), lr=1e-3)
...
for episode in tqdm(range(num_episodes)):
    ...
    for t in range(simul.target_time - simul.start_time):
        ...
        context = simul.run()
        if context is not None:
            pred_pathtime = context['path_time_TimeMachine']

            # preprocessing of features
            context_df =  pd.Series(context).to_frame().T
            context_df['remain_time_from_TimeMachine'] = simul.req_leaving_time_TimeMachine - simul.cur_time
            contexts_feature_df = make_feature_set_embedding(context_df,
                                    temporal_feature_cols, spatial_feature_cols, other_feature_cols, fillna=0)
            contexts_tensor = torch.tensor(contexts_feature_df.to_numpy()).to(device)
            
            # select action
            action = None
            with torch.no_grad():
                mu0, logvar0 = apicall_0_model(contexts_tensor)
                logvar0 = torch.clamp(logvar0, min=-10, max=10)   # clamp
                ucb0 = (mu0 + alpha * torch.exp(0.5*logvar0)).mean()

                mu1, logvar1 = apicall_1_model(contexts_tensor)
                logvar1 = torch.clamp(logvar1, min=-10, max=10)   # clamp
                ucb1 = (mu1 + alpha * torch.exp(0.5*logvar1)).mean()
                
                action = 0 if ucb0.item() >= ucb1.item() else 1

            # save data for update action
            contexts_arr.append(contexts_feature_df.to_numpy().squeeze())
            actions.append(action)
            ...
            if context['path_time_LastAPI'] is not None:
                context_df =  pd.Series(context).to_frame().T
                context_df['remain_time_from_TimeMachine'] = simul.req_leaving_time_TimeMachine - simul.cur_time
                contexts_feature_df = make_feature_set_embedding(context_df,
                                        temporal_feature_cols, spatial_feature_cols, other_feature_cols, fillna=0)
                ...
                path_time_LastAPI = context_df['path_time_LastAPI'].item()

                with torch.no_grad():
                    path_delta = model_pathtime(contexts_tensor)
                    path_delta = torch.clamp(path_delta, min=-path_time_LastAPI, max=1440)
                    pred_pathtime = path_time_LastAPI + path_delta.mean().item()
                    
                    path_time_LastAPIs.append(path_time_LastAPI)
                    pred_pathtimes.append(pred_pathtime)
                calc_pathtimes_TF.append(True)
            else:
                calc_pathtimes_TF.append(False)
        
        if simul.cur_time + pred_pathtime > simul.target_time - 8:
            adjcent_matrix = simul.temporal_graph.transform(simul.cur_time)
            true_leaving_info = simul.api_call(adjcent_matrix, save_instance=False, save_history=False)
            break
    ...
    len(contexts_arr)
    len(actions)
    len(calc_pathtimes_TF)
    len(path_time_LastAPIs)
    len(pred_pathtimes)
    
    # END OF EPISODES ------------------------------------------------------
    # accrate path time reward
    pred_leaving_time = int(np.ceil(simul.target_time - 8 - (pred_pathtime)))
    reward_x = (pred_leaving_time + true_leaving_info['path_time']) - simul.target_time
    reward = reward_function(reward_x)
    ...
    if action0_TF.sum() > 0:
        x_train = torch.tensor(np.stack(contexts_arr)[action0_TF]).to(device)
        y_train = torch.tensor(np.stack(total_reward)[action0_TF].reshape(-1,1)).to(device)

        apicall_0_model.train()
        optimizer_apicall_0.zero_grad()
        mu, logvar = apicall_0_model(x_train)
        logvar = torch.clamp(logvar, min=-10, max=10)
        std = torch.exp(0.5*logvar)
        loss = torch.nn.functional.gaussian_nll_loss(mu, y_train, std**2)
        loss.backward()
        optimizer_apicall_0.step()
        with torch.no_grad():
            losses[0] = loss.item()
            action0[0] = mu.mean().item()
            action0[1] = std.mean().item()
        
    if action1_TF.sum() > 0:
        x_train = torch.tensor(np.stack(contexts_arr)[action1_TF]).to(device)
        y_train = torch.tensor(np.stack(total_reward)[action1_TF].reshape(-1,1)).to(device)
        
        apicall_1_model.train()
        optimizer_apicall_1.zero_grad()
        mu, logvar = apicall_1_model(x_train)
        logvar = torch.clamp(logvar, min=-10, max=10)
        std = torch.exp(0.5*logvar)
        loss = torch.nn.functional.gaussian_nll_loss(mu, y_train, std**2)
        loss.backward()
        optimizer_apicall_1.step()
        with torch.no_grad():
            losses[1] = loss.item()
            action1[0] = mu.mean().item()
            action1[1] = std.mean().item()

    # path_time model update ----------------------------------------------------------------------
    if sum(calc_pathtimes_TF) > 0:
        max_pathtime = max(path_time_LastAPIs)
        x_train = torch.tensor(np.stack(contexts_arr)[calc_pathtimes_TF]).to(device)
        y_train = torch.tensor(true_leaving_info['path_time'] - np.array(path_time_LastAPIs), dtype=torch.float32).view(-1,1).to(device)

        model_pathtime.train()
        optimizer_pathtime.zero_grad()
        path_delta = model_pathtime(x_train)
        path_delta = torch.clamp(path_delta, min=-max_pathtime, max=1440)
        loss = loss_mse(path_delta, y_train)
        loss.backward()
        optimizer_pathtime.step()
        with torch.no_grad():
            losses[2] = loss.item()
    ...
    with torch.no_grad():
        # history
        history_t['n_of_rounds'] = len(actions)
        history_t['last_api_rounds'] = np.where(np.array(actions)==1)[0].max() if np.sum(actions) > 0 else 0
        history_t['last_api_loc_ratio'] = history_t['last_api_rounds'] / len(actions)
        history_t['contexts'] = contexts_arr
        history_t['actions'] = actions
        history_t['pred_leaving_time'] = pred_leaving_time
        history_t['pred_pathtime'] = pred_pathtime
        history_t['true_pathtime'] = true_leaving_info['path_time']
        history_t["reward_x"] = reward_x
        history_t['total_reward'] = total_reward
        history_t["reward"] = reward
        history_t["reward_regualize"] = reward_regualize
        ...
        # summary
        action0_round = [(np.round(a, 2) if a is not None else None) for a in action0]
        action1_round = [(np.round(a, 2) if a is not None else None) for a in action1]
        losses_round = [(np.round(l, 3) if l is not None else None) for l in losses]
        summary_of_episodes = f"loss (act0/act1/pathtime: {losses_round},\t reward_x: {reward_x:.2f},\t reward: {reward:.3f},\t #_of_act: {sum(action1_TF)},\t last_api_ratio: {history_t['last_api_loc_ratio']:.2f},\t [μ,V]act0/1: {action0_round} / {action1_round}"
        print(f"\r  → {summary_of_episodes}")
        ...
        # path = r'D:\DataScience\SNU_DataScience\OhLAB\CLabProject'
        path = "/content/drive/MyDrive/SNU_OhLAB/CLabProject"
        cPickle.dump(apicall_0_model.state_dict(), open(f"{path}/240921_Alg1_5_apiModel_0.pkl", 'wb'))
        cPickle.dump(apicall_1_model.state_dict(), open(f"{path}/240921_Alg1_5_apiModel_1.pkl", 'wb'))
        cPickle.dump(model_pathtime.state_dict(), open(f"{path}/240921_Alg1_5_pathtimeModel.pkl", 'wb'))
        cPickle.dump(history, open(f"{path}/240921_Alg1_5_history.pkl", 'wb'))
...
from six.moves import cPickle
path = 'D:\DataScience\SNU_DataScience\SNU_OhLAB\CLabProject\Model'
# path = "/content/drive/MyDrive/SNU_OhLAB/CLabProject/Model"
history = cPickle.load(open(f"{path}/240921_Alg1_5_history.pkl", 'rb'))
history[0].keys()

dist_y = [np.sum(hist['actions']) for hist in history]
dist_y = [np.sum(hist['actions'])/hist['n_of_rounds'] for hist in history]
dist_y = [hist['reward_x'] for hist in history]
dist_y = [hist['reward'] for hist in history]
dist_y = [hist['true_leaving_info']['path_time'] - hist['pred_path_time_mu'] for hist in history]
dist_y = [hist['true_pathtime'] - hist['pred_pathtime'] for hist in history]
...
# visualize
plt.figure(figsize=(20,3))
plt.scatter(np.arange(len(history)), dist_y, edgecolor='white', alpha=0.7)
plt.plot(np.arange(len(history)), dist_y, alpha=0.3)
plt.axhline(0, c='black', alpha=0.3)
plt.axhline(-13, c='red', alpha=0.3)
# plt.yscale("symlog")
plt.show()
...
for epoch in range(num_epochs):
    ...
    pathtime_diff_tensor = torch.tensor(pathtime_diff).to(device)

    optimizer.zero_grad()
    logvar = model(contexts_feature_tensor)
    # logvar = torch.clamp(logvar, min=-20, max=20)
    std = torch.exp(0.5*logvar.mean(dim=1, keepdims=True))
    mu = torch.zeros_like(std).to(device)
...
# load_path = r'D:\DataScience\SNU_DataScience\SNU_OhLab\CLab_Project\Model'
# load_path = r'D:\DataScience\SNU_DataScience\OhLAB\CLabProject\Model'
history = cPickle.load(open(f"{load_path}/240906_Alg3_1_DirectEnsemble_history.pkl", 'rb'))
# print('load_history')
len(history)
