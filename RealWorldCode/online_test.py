...
model_py_file = 'Algorithm3_Model_json'
# model_py_file = 'Demo_OnlinePrediction'
...
class TimeCheckerAuto:
    ...
    def initialize(self):
        ...
        # 1200초(20분) ~ 10800(180분)
        duration = random.randint(1200,10800)
                ...
                [
                    {
                        ...
                        "req_leaving_time": self.leaving_time.strftime("%Y-%m-%dT%H:%M:%S%z"),
...
def run_time_checker(appkey, sleep_condition=False):
    lock_path = "/home/kimds929/BaroNowProject/result.lock"  # 파일 잠금을 위한 경로
    ...
    while True:
        try:
            ...
            if not pd.isna(last_context['path_time_LastAPI']):
                ...
                with lock:  # 파일 잠금 시작
                    with open("/home/kimds929/BaroNowProject/result.txt", "a") as file:
...
def start_threaded_checkers(appkey, num_threads, sleep_condition=False):
    threads = []
    
    for i in range(num_threads): 
        # print(appkey)
        t = threading.Thread(target=run_time_checker, args=(appkey, sleep_condition))
        threads.append(t)
        t.start()
        print(t)
        time.sleep(3)
...
if __name__ == "__main__":
    # 여러 스레드에서 run_time_checker 실행
    appKeys =  [('jICW6XpKkB9hSdzIJrySU4EtcJ7JzDTZBBPDeHo7', 'bfeae872ac9a35817ff18366aeb852dd')] 
    start_threaded_checkers(appkey=appKeys[0], num_threads=5, sleep_condition=False)
