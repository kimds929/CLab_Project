...
#####################################분리###########################################
            # our model
            baronow_prediction = BaroNowPrediction(model='transformer')
            # rulebased
...
#########################확인용 저장#######################################################################################################################################
                if episode ==0:
                    csv_file='/home/baronow/BaroNowProject_V3/online_result/transformer/kakao/kakao_status0.csv'
                    # 파일이 이미 존재하는지 확인
                    ...
                    csv_file='/home/baronow/BaroNowProject_V3/online_result/transformer/tmap/tmap_status0.csv'
                    # 파일이 이미 존재하는지 확인
                    if os.path.exists(csv_file):
                        # 기존 파일에 데이터 추가 (header=False로 기존 헤더를 덮어쓰지 않음)
...
# ---------------------------------------------------------------------------------------
                ...
                if output_kakao['status'] == 1 and output_tmap['status'] == 1:   # 출발
                    ...
                    csv_file='/home/baronow/BaroNowProject_V3/online_result/transformer/kakao/output_kakao.csv'
                    # 파일이 이미 존재하는지 확인
                    if os.path.exists(csv_file):
                        # 기존 파일에 데이터 추가 (header=False로 기존 헤더를 덮어쓰지 않음)
                    ...
                    csv_file='/home/baronow/BaroNowProject_V3/online_result/transformer/tmap/output_tmap.csv'
                    # 파일이 이미 존재하는지 확인
                    if os.path.exists(csv_file):
                        # 기존 파일에 데이터 추가 (header=False로 기존 헤더를 덮어쓰지 않음)
...
# ---------------------------------------------------------------------------------------                
                if output_kakao['status'] == 2:  # 새로운 에피소드로 계산
                    ...
                    api_count_kakao += 1
                    #################확인용#####################
                    csv_file='/home/baronow/BaroNowProject_V3/online_result/transformer/kakao/kakao_status2.csv'
                    # 파일이 이미 존재하는지 확인
                    if os.path.exists(csv_file):
                        # 기존 파일에 데이터 추가 (header=False로 기존 헤더를 덮어쓰지 않음)
                        data_api_kakao.to_csv(csv_file, mode='a', header=False, index=False)
...
# --------------------------------------------------------------------------------------- 
                if output_tmap['status'] == 2:  # 새로운 에피소드로 계산
                    ...
                    #################확인용#####################
                    csv_file='/home/baronow/BaroNowProject_V3/online_result/transformer/tmap/tmap_status2.csv'
                    # 파일이 이미 존재하는지 확인
                    if os.path.exists(csv_file):
                        # 기존 파일에 데이터 추가 (header=False로 기존 헤더를 덮어쓰지 않음)
                        data_api_tmap.to_csv(csv_file, mode='a', header=False, index=False)
            ...
            if output_kakao['status'] ==1 and output_tmap['status'] ==1:
                ...
                if not pd.isna(last_context_kakao['path_time_LastAPI']):
                    ...
                    with open("/home/baronow/BaroNowProject_V3/online_result/transformer/result.txt", "a") as file:  # 'a' 모드는 파일에 덧붙여 쓰기
